										...
										{filteredBooks.length > 0 ? (
											filteredBooks.map((book, index) => (
												<tr key={book._id}>
													...
													<td>{index + 1}</td>
													<td><a href="#" className="fw-semibold">{book.title}</a></td>
													<td>{book.author}</td>
													<td>{book.category}</td>
													<td>{book.publishedYear}</td>
													<td>
														<span className={`badge ${book.availability ? 'bg-success' : 'bg-danger'}`}>
															{book.availability ? 'Available' : 'Not Available'}
														</span>
													</td>
													<td>
														<Link to={`/books/${book._id}`} className="btn btn-sm btn-light">
															<RiEyeLine />
														</Link>
														<button 
															type="button" 
