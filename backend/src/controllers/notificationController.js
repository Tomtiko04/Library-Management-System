...
// 				(book) => book.dueDate <= new Date(Date.now() + 3 * 24 * 60 * 60 * 1000)
// 			);
...
// TODO: The waitlist i worked on
// To send notification to the first person in the wait list
exports.notifyNextUser = async (bookId) => {
	try {
		const nextInLine = await Waitlist.findOne({ book: bookId, status: "waiting" }).sort({
			joinedAt: 1,
		});
		...
		if (nextInLine) {
			...
			await sendEmail({
				...
				text: `Dear ${user.name},\n\nThe book "${bookId}" is now available. Please borrow it within 24 hours or your reservation will expire.\n\nThank you.`,
			});
		}
	} catch (error) {
		res.status(500).json({
...
exports.expireReservations = async () => {
	try {
		const expired = await Waitlist.updateMany(
			{
				status: "notified",
				notifiedAt: { $lte: new Date(Date.now() - 24 * 60 * 60 * 1000) },
			},
			{ $set: { status: "expired" } }
		);
		console.log(`${expired.modifiedCount} reservations expired`);
	} catch (error) {
		res.status(500).json({
			status: "error",
			message: error.message,
